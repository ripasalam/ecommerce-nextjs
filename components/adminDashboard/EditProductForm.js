...
const EditProductForm = () => {
            ...
            axios.get(`${process.env.NEXT_PUBLIC_BASE_URL}/products/${productId}`)
                .then((res) => {
                    ...
                    if (!ignore) {
                        ...
                        setInput({
                            nameProduct: res.data.product.name,
                            description: res.data.product.description,
                            price: res.data.product.price,
                            quantity: res.data.product.quantity,
                            category: res.data.product.categoryId
                        })
                        setSize(res.data.product.sizes)
                        setFile(res.data.product.image)
                    }
                }).catch((error) => {
                    console.log(error)
                })
    ...
    const handleEditProduct = (e) => {
        ...
        axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/products/${productId}`, { input, sizes }, {
            ...
            <div className='px-6 py-3 mb-3'>
                ...
                <div>
                    <form onSubmit={handleEditProduct} className="space-y-4 md:space-y-6" >
                        ...
                        <button type='submit' className=" w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Edit Product</button>
                    </form>
                </div>
            </div>
        </div>
